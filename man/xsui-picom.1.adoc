...
SYNOPSIS
--------
*xsui-picom* [_OPTIONS_]

DESCRIPTION
-----------
xsui-picom is a compositor based on Dana Jansens' version of xcompmgr (which itself was written by Keith Packard). It includes some improvements over the original xcompmgr, like window frame opacity and inactive window transparency.

Background blur and shadow are never applied to the following windows, regardless of configuration: popup menus, dropdown menus, top-level menus, tooltips, notifications, dialogs, and override-redirect windows. Additionally, background blur and shadow are not applied to windows with opacity 5% or lower. These are hardcoded behavioral changes and cannot be configured.
...
*-c*, *--shadow*::
	Enabled client-side shadows on windows. Note desktop windows (windows with __NET_WM_WINDOW_TYPE_DESKTOP_) never get shadow, unless explicitly requested using the wintypes option. Popup menus, dropdown menus, tooltips, notifications, dialogs, and override-redirect windows also never get shadow. Windows with opacity 5% or lower are also excluded from shadowing.
...
*--config* _PATH_::
	Look for configuration file at the path. See xref:_configuration_files[*CONFIGURATION FILES*] section below for where xsui-picom looks for a configuration file by default. Use `/dev/null` to avoid loading configuration file.

*--write-pid-path* _PATH_::
	Write process ID to a file. It is recommended to use an absolute path.

*--plugins* _PATH_::
	Specify plugins to load. Plugins will first be searched in current working directory (unless specified in the config file, in which case this step is skipped), then in `$XDG_CONFIG_HOME/xsui-picom/plugins`, then in `$XDG_CONFIG_DIRS/xsui-picom/plugins`. If all of the above fail, the plugin name is passed directly to the dynamic loader. Can be specified multiple times to load more than one plugins.
...
[[fullscreen-blacklist]]*--fullscreen-blacklist* _CONDITION_::
	Conditions of windows that should not be considered full-screen even when they set the EWMH fullscreen state or cover the whole screen. Useful for full-screen launcher apps that intentionally take up the whole screen but still want the compositor to keep running so they can be painted with transparency/blur effects. Using this is discouraged, see the xref:_window_rules[*WINDOW RULES*] section for the recommended way to set window-specific fullscreen behavior. Note the obsolete system apps SmartSlide and sui-panel are already hardcoded so they are never considered fullscreen; this option is for adding additional apps at runtime.
...
*--blur-background*::
	Blur background of semi-transparent / ARGB windows. Bad in performance, with driver-dependent behavior. The name of the switch may change without prior notifications. Note popup menus, dropdown menus, tooltips, notifications, dialogs, and override-redirect windows are never blurred. Windows with opacity 5% or lower are also excluded from background blur.

*--blur-background-frame*::
	Blur background of windows when the window frame is not opaque.  Implies *--blur-background*. Bad in performance, with driver-dependent behavior. The name may change. Note popup menus, dropdown menus, tooltips, notifications, dialogs, and override-redirect windows are never blurred. Windows with opacity 5% or lower are also excluded.
...
*--xrender-sync-fence*::
	Use X Sync fence to sync clients' draw calls, to make sure all draw calls are finished before xsui-picom starts drawing. Needed on nvidia-drivers with GLX backend for some users.
...
<<shadow-ignore-shaped>>, <<inactive-opacity>>, <<active-opacity>>, <<inactive-opacity-override>>, <<inactive-dim>>, <<mark-wmwin-focused>>, <<mark-ovredir-focused>>, <<invert-color-include>>, <<shadow-exclude>>, <<fade-exclude>>, <<focus-exclude>>, <<rounded-corners-exclude>>, <<blur-background-exclude>>, <<opacity-rule>>, <<corner-radius-rules>>, <<window-shader-fg-rule>>, <<clip-shadow-above>>, <<unredir-if-possible-exclude>>, <<fullscreen-blacklist>>. As well as the xref:wintypes[*wintypes*] configuration file option.
...
+
NOTE: Setting _opacity_ explicitly with a rule will override the opacity value from the window properties (i.e. _pass:[_]NET_WM_WINDOW_OPACITY_), which is used by tools like `xsui-picom-trans` for setting the opacity of window. If you would like to keep using tools like `xsui-picom-trans`, you can choose to set the opacity only for windows without the opacity property by matching `! _NET_WM_WINDOW_OPACITY`.
...
xsui-picom supports xref:fading[fading] animation when you open or close a window. In addition to that, xsui-picom also has a very powerful animation script system, which can be used to animate many aspects of a window based on certain triggers. Animation scripts can be defined in your configuration file by setting the option _animations_. It is also possible to define animations per-window using the xref:_window_rules[*WINDOW RULES*] system, by setting the <<window-rules-animations>> option in a rule. (Read the rest of this section first before you go there.)
...
Defining an animation is a bit involved. To make animations more approachable, without you having to learn the nitty-gritty details of the script system, xsui-picom provides a number of presets that you can use by just specifying a handful of options.
...
ifndef::env-web[]
NOTE: Describing animations with only words is difficult. We have short video clips showing off each preset, but sadly they cannot be included in this manpage. The web version of this document hosted on our website at https://xsui-picom.app[] on the other hand, does have those clips.
endif::[]
...
A context variable is a variable xsui-picom defines for you, and you can use them in expressions like any other variables. Their values reflect certain attributes of the window you are animating.
...
If you have created an animation script that you think is particularly cool, you are encouraged to share it with the community. You can submit an issue or a pull request to xsui-picom on GitHub, and get a chance to have your animation included as one of the presets, so it can be used by everyone.

SHADER INTERFACE
----------------

This section describes the interface of a custom shader, how it is used by xsui-picom, and what parameters are passed by xsui-picom to the shader.

A custom shader is a GLSL fragment shader program, which can be used to override the default way of how a window is rendered. If a custom shader is used, the default xsui-picom effects (e.g. dimming, color inversion, etc.) will no longer be automatically applied. It would be the custom shader's responsibility to apply these effects.

The interface between xsui-picom and a custom shader is dependent on which backend is being used. The xrender backend doesn't support shader at all. Here we describe the interface provided by the glx backend.

The shader must define a function, _vec4 window_shader()_, which would be the entry point of the shader. The returned _vec4_ will be used to set __gl_FragColor__. A function, _vec4 default_post_processing(vec4 c)_, is provided for applying the default xsui-picom effects to input color 'c'.
...
The default behavior of xsui-picom window rendering can be replicated by the following shader:
...
CONFIGURATION FILES
-------------------
xsui-picom could read from a configuration file if libconfig support is compiled in. If *--config* is not used, xsui-picom will seek for a configuration file in `$XDG_CONFIG_HOME/xsui-picom.conf` (`~/.config/xsui-picom.conf`, usually), then `$XDG_CONFIG_HOME/xsui-picom/xsui-picom.conf`, then `$XDG_CONFIG_DIRS/xsui-picom.conf` (often `/etc/xdg/xsui-picom.conf`), then `$XDG_CONFIG_DIRS/xsui-picom/xsui-picom.conf`.

When `@include` directive is used in the config file, xsui-picom will first search for the included file in the parent directory of `xsui-picom.conf`, then in `$XDG_CONFIG_HOME/xsui-picom/include/`, then in `$XDG_CONFIG_DIRS/xsui-picom/include`.

xsui-picom uses general libconfig configuration file format. A sample configuration file is available as `xsui-picom.sample.conf` in the source tree. Most of command line switches can be used as options in configuration file as well. For example, *--vsync* option documented above can be set in the configuration file using `vsync = `. Command line options will always overwrite the settings in the configuration file.
...
The following window types are always treated as overlay windows and are excluded from background blur and shadow, regardless of configuration: `popup_menu`, `dropdown_menu`, `menu`, `tooltip`, `notification`, `dialog`, and `normal` windows with override-redirect. This is a hardcoded behavioral change and cannot be configured.

Additionally, windows with opacity 5% (0.05) or lower are excluded from background blur and shadow. This is a hardcoded behavioral change and cannot be configured.

Following per window-type options are available: ::

   fade, shadow:::
     Controls window-type-specific shadow and fade settings. Shadow is never drawn for popup menus, dropdown menus, tooltips, notifications, dialogs, and override-redirect windows, regardless of this setting. Also, shadow is never drawn for windows with opacity 5% or lower.
  ...
  focus:::
   ...
   blur-background:::
     Controls whether the window of this type will have its transparent background blurred. Note this setting is ignored for popup menus, dropdown menus, tooltips, notifications, dialogs, and override-redirect windows, which are never blurred regardless of this setting. Also ignored for windows with opacity 5% or lower.
...
* xsui-picom reinitializes itself upon receiving `SIGUSR1`.

D-BUS API
---------

It's possible to control xsui-picom via D-Bus messages, by running xsui-picom with *--dbus* and send messages to `com.github.yshui.xsui-picom.<DISPLAY>`. `<DISPLAY>` is the display used by xsui-picom, with all non-alphanumeric characters transformed to underscores. For `DISPLAY=:0.0` you should use `com.github.yshui.xsui-picom._0_0`, for example.
...
+
------------
$ xsui-picom --config /dev/null
------------

* Run xsui-picom with client-side shadow and fading:
+
------------
$ xsui-picom -cf
------------
...
+
------------
$ xsui-picom -bcf -i 0.8 -e 0.8 --no-fading-openclose
------------

* Draw white shadows:
+
------------
$ xsui-picom -c --shadow-red 1 --shadow-green 1 --shadow-blue 1
------------

* Avoid drawing shadows on wbar window:
+
------------
$ xsui-picom -c --shadow-exclude 'class_g = "wbar"'
------------

* Enable VSync with GLX backend:
+
------------
$ xsui-picom --backend glx --vsync
------------

BUGS
----
Please submit bug reports to <https://github.com/yshui/xsui-picom>.
...
SEE ALSO
--------
*xcompmgr*(1), xref:xsui-picom-inspect.1.adoc[*xsui-picom-inspect*(1)], xref:xsui-picom-trans.1.adoc[*xsui-picom-trans*(1)]
